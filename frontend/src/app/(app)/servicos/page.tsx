...
export default function ServicosPage() {
  ...
  async function handleUpdate(event: React.FormEvent<HTMLFormElement>) {
    ...
    setError('');
    try {
      const res = await fetch(`${API_URL}/services/${editingId}`, {
        method: 'PATCH',
        headers: {
          'Content-Type': 'application/json',
          ...(token ? { Authorization: `Bearer ${token}` } : {}),
      ...
      setError('Nao foi possivel atualizar servico.');
    }
  }
  ...
  async function handleDeactivate(id: string) {
    ...
    try {
      ...
      setError('Nao foi possivel inativar servico.');
    }
  }
  ...
  return (
    <section className="ip-page" style={{ display: 'grid', gap: 16 }}>
        ...
        <div className="panel-header">
          <div className="toolbar ip-toolbar">
            <div style={{ display: 'grid', gap: 6 }}>
              <h1 className="ip-page-title">Servicos</h1>
              <span className="ip-page-subtitle">Catalogo e gestao dos servicos cadastrados.</span>
            </div>
            <div className="toolbar-actions">
              <button
                ...
                onClick={() => {
                  ...
                  const form = document.getElementById('novo-servico');
                  if (form) form.scrollIntoView({ behavior: 'smooth', block: 'start' });
                }}
              >
                {formOpen ? 'Fechar' : '+ Novo Servico'}
              </button>
            </div>
          </div>
        </div>
      ...
      {formOpen ? (
        <div className="panel">
          <div className="panel-header">
            <strong>{editingId ? 'Editar Servico' : 'Novo Servico'}</strong>
          </div>
